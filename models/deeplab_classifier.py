...
class deeplabClassifier(BaseModel):
    ...
    def backward_D(self):
        ...
        pred_real = self.netD.forward(self.predic_B)
        loss_D_real = self.criterionAdv(pred_real, False)

        self.A_label_onehot = Variable(self.A_label_onehot.data)
        pred_fake = self.netD.forward(self.A_label_onehot)
        loss_D_fake = self.criterionAdv(pred_fake, True)
