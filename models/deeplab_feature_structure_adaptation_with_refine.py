...
class deeplabGanStructureAdaptation(BaseModel):
    ...
    def initialize(self, args):
        ...
        self.optimizer_D_S = torch.optim.Adam(self.netD_structure.parameters(),
                                            lr=args['lr_gan'], betas=(args['beta1'], 0.999))
        self.optimizer_G_G_S = torch.optim.SGD(itertools.chain(self.netG.parameters(), self.netG_structure.parameters()), lr=args['lr_refine'], momentum=0.9, weight_decay=5e-4)
    ...
    def backward_P(self):
        self.input_deeplabPart2 = self.deeplabPart1(self.A)
        self.input_A_G = Variable(self.input_deeplabPart2.data)
        self.feature_A = self.deeplabPart2(self.input_deeplabPart2)
    ...
    def backward_G(self):
        ...
        self.resdual_B = self.netG(self.input_B_G)
        self.feature_B = self.deeplabPart2(self.input_B_G) + self.resdual_B
    ...
    def backward_G_S(self):
        self.resdual_B_S = self.netG_structure(self.feature_B_input)
        self.predict_B = self.deeplabPart3(self.feature_B_input) + self.resdual_B_S
    ...
    def backward_G_G_S(self):
        self.feature_A_with_G = self.netG(self.input_A_G) + self.feature_A_input
        self.predic_A_with_G = self.netG_structure(self.feature_A_with_G) + self.deeplabPart3(self.feature_A_with_G)
        self.loss_refine = self.criterionCE(self.feature_A_with_G, self.input_A_label)
    ...
    def optimize_parameters(self):
        ...
        # G and G_S
        self.optimizer_G_G_S.zero_grad()
        self.backward_G_G_S()
        self.optimizer_G_G_S.step()
